...
public class RequestLogin extends GameRequest {
    ...
    public void parse() throws IOException {
        ...
        player_id = DataReader.readString(dataInput).trim();
        password = DataReader.readString(dataInput).trim();
    }

    @Override
    public void doBusiness() throws Exception {
        Log.printf("User '%s' is connecting to the server...", player_id);
        Player player = null;
        ...
        if (version.compareTo(Constants.CLIENT_VERSION) >= 0) {
            if (!player_id.isEmpty()) {
                ...
                // Let's make a fake user for showing a connection demo -- without proper DB set tup.q
                Log.printf("User '%s' entered passwd '%s'", player_id, password);
                if(player_id.equals("test") && password.equals("fg"))
                    player = new Player(100, "test", "fg", (short) 1, 1000);
                else
                    player = null;
            }
            if (player == null) {
                responseLogin.setStatus((short) 1); // User info is incorrect
                Log.printf("User '%s' has failed to log in.", player_id);
            } else {
                ...
                if (client.getPlayer() == null || player.getID() != client.getPlayer().getID()) {
                    ...
                    if (thread != null) {
                        ...
                        thread.newSession();
                        Log.printf("User '%s' account is already in use.", player_id);
                    } else {
